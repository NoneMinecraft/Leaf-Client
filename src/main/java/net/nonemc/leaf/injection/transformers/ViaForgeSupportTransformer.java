...
import net.nonemc.leaf.utils.misc.ASMUtils;
...
public class ViaForgeSupportTransformer implements IClassTransformer {
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        if (name.equals("net.nonemc.leaf.injection.forge.mixins.network.MixinNetworkManager")) {
            try {
                ...
                classNode.methods.stream().filter(methodNode -> methodNode.name.equals("createNetworkManagerAndConnect")).forEach(methodNode -> {
                    for (int i = 0; i < methodNode.instructions.size(); ++i) {
                        ...
                        if (abstractInsnNode instanceof TypeInsnNode) {
                            ...
                            if (tin.desc.equals("net/nonemc/leaf/injection/forge/mixins/network/MixinNetworkManager$1")) {
                            ...
                            if (min.owner.equals("net/nonemc/leaf/injection/forge/mixins/network/MixinNetworkManager$1") && min.name.equals("<init>")) {
