package net.nonemc.leaf.ui.clickgui.style.styles;
...
import static net.nonemc.leaf.libs.block.BlockLib.getBlockName;
...
public class LiquidBounceStyle extends Style {
    ...
    public void drawPanel(int mouseX, int mouseY, Panel panel) {
        ...
        if(panel.getScrollbar() && panel.getFade() > 0) {
            ...
            RenderUtils.drawRect(panel.getX() - 2, panel.getY() + 30 + (panel.getFade() - 24F) / (panel.getElements().size() - Leaf.moduleManager.getModule(ClickGUIModule.class).maxElementsValue.get()) * panel.getDragged() - 10.0f, panel.getX(), panel.getY() + 40 + (panel.getFade() - 24.0f) / (panel.getElements().size() - Leaf.moduleManager.getModule(ClickGUIModule.class).maxElementsValue.get()) * panel.getDragged(), Integer.MIN_VALUE);
        }
    }
            ...
            if(moduleElement.isShowSettings()) {
                ...
                for(final Value value : moduleValues) {
                    ...
                    if (value instanceof BoolValue) {
                        ...
                        final String text = value.getName() + "§f: §c" + (value instanceof BlockValue ? getBlockName(integerValue.get()) + " (" + integerValue.get() + ")" : integerValue.get());
                        final float textWidth = Fonts.font35.getStringWidth(text);
