...
object ToolDropdown {
    ...
    private val expandIcon = ResourceLocation("leaf/misc/expand.png")
    ...
    fun drawToggleSwitch(x: Float, y: Float, width: Float, height: Float, state: Boolean) {
        ...
        if (state)
            RenderUtils.drawFilledCircle(
                x.toInt() + width.toInt() - 2 - (height.toInt() - 4) / 2,
                y.toInt() + 2 + (height.toInt()- 4) / 2,
                (height - 4F) / 2F,
                Color(24, 24, 24)
            )
        else
            RenderUtils.drawFilledCircle(
                x.toInt() + 2 + (height.toInt() - 4) / 2,
                y.toInt() + 2 + (height.toInt() - 4) / 2,
                (height - 4F) / 2F,
                Color(160, 160, 160)
            )
