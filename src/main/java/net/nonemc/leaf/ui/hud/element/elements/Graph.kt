...
import net.nonemc.leaf.libs.packet.PacketLib.avgInBound
import net.nonemc.leaf.libs.packet.PacketLib.avgOutBound
...
class Graph(
    ...
    // general
    private val graphValue =
        ListValue("Graph-Value", arrayOf("Speed", "RotationSpeed", "Packet-In", "Packet-Out"), "Speed")
    ...
    override fun updateElement() {
        ...
        ).toFloat() * 20F * mc.timer.timerSpeed
    ...
    override fun drawElement(partialTicks: Float): Border {
        ...
        if (timer.hasTimePassed(updateDelay.get().toLong())) {
            when (graphValue.get().lowercase(Locale.getDefault())) {
                "speed" -> valueStore.add(EntityMoveLib.getSpeed() * 10F)
                "RotationSpeed" -> valueStore.add(rotationSpeed)
                "packet-in" -> valueStore.add(avgInBound.toFloat())
                "packet-out" -> valueStore.add(avgOutBound.toFloat())
            }
