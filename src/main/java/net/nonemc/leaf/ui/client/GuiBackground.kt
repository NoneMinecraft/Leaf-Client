...
class GuiBackground(val prevGui: GuiScreen) : GuiScreen() {
    ...
    override fun actionPerformed(button: GuiButton) {
        when (button.id) {
            ...
            3 -> {
                ...
                try {
                    Files.copy(file.toPath(), FileOutputStream(Leaf.fileManager.backgroundFile))

                    val image = ImageIO.read(FileInputStream(Leaf.fileManager.backgroundFile))
                    ...
                    Leaf.fileManager.backgroundFile.delete()
                }
            }
            4 -> {
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        mc.fontRendererObj.drawCenteredString("%ui.background.gcurrent%: " + if(Leaf.background == null) { GradientBackground.nowGradient.name } else { "Customized" },
