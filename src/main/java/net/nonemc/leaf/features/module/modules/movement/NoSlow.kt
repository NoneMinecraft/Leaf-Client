...
object NoSlow : Module() {
    ...
    private fun sendPacket2(packetType: String) {
        ...
        when (packetType.lowercase()) {
            ...
            "Intave" -> {
                if (start) PacketLib.sendPacketNoEvent(
    ...
    fun onMotion(event: MotionEvent) {
        ...
        if ((!EntityMoveLib.isMoving() && onlyMove.get()) || (onlyGround.get() && !mc.thePlayer.onGround)) {
            return
        }
        ...
        if ((blockModifyValue.get() && (mc.thePlayer.isBlocking) && heldItem is ItemSword)
            ...
            when (modeValue.get().lowercase()) {
                ...
                "intavesword" -> {
                    if (start) PacketLib.sendPacketNoEvent(C08PacketPlayerBlockPlacement(mc.thePlayer.heldItem))
                }
                ...
                "switchitem" -> {
                    PacketLib.sendPacketNoEvent(C09PacketHeldItemChange(mc.thePlayer.inventory.currentItem % 8 + 1))
                    PacketLib.sendPacketNoEvent(C09PacketHeldItemChange(mc.thePlayer.inventory.currentItem))
                }
        ...
        if ((modeValue.equals("Matrix") || modeValue.equals("GrimAC")) && (lastBlockingStat || isBlocking)) {
            if (msTimer.hasTimePassed(230) && nextTemp) {
                ...
                if (modeValue.equals("GrimAC")) {
                    PacketLib.sendPacketNoEvent(C09PacketHeldItemChange((mc.thePlayer.inventory.currentItem + 1) % 9))
                    PacketLib.sendPacketNoEvent(C09PacketHeldItemChange(mc.thePlayer.inventory.currentItem))
                } else {
                    PacketLib.sendPacketNoEvent(
                        C07PacketPlayerDigging(
                ...
                if (packetBuf.isNotEmpty()) {
                    ...
                    for (packet in packetBuf) {
                        ...
                        if (!((packet is C02PacketUseEntity || packet is C0APacketAnimation) && !canAttack)) {
                            PacketLib.sendPacketNoEvent(packet)
                        }
                    }
                    packetBuf.clear()
                }
            ...
            when (teleportModeValue.get().lowercase()) {
                ...
                "custom" -> {
                    if (EntityMoveLib.isMoving()) {
                        EntityMoveLib.strafe(teleportCustomSpeedValue.get())
                    }
