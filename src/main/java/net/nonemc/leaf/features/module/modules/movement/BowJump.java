...
public class BowJump extends Module {
    ...
    public void onEnable() {
        ...
        EntityMoveLib.INSTANCE.strafe(0.0f);
        ...
        switch (bowState) {
            case 0:
                ...
                if (slot < 0 || !mc.thePlayer.inventory.hasItem(Items.arrow)) {
                    ...
                    if (lastSlot != slot) PacketLib.INSTANCE.sendPacketNoEvent(new C09PacketHeldItemChange(slot));
                    PacketLib.INSTANCE.sendPacketNoEvent(new C08PacketPlayerBlockPlacement(new BlockPos(-1, -1, -1), 255, mc.thePlayer.inventoryContainer.getSlot(slot + 36).getStack(), 0, 0, 0));
            ...
            case 1:
                ...
                if (mc.thePlayer.ticksExisted - lastPlayerTick > delayBeforeLaunch.get()) {
                    PacketLib.INSTANCE.sendPacketNoEvent(new C03PacketPlayer.C05PacketPlayerLook(mc.thePlayer.rotationYaw, -90, mc.thePlayer.onGround));
                    PacketLib.INSTANCE.sendPacketNoEvent(new C07PacketPlayerDigging(C07PacketPlayerDigging.Action.RELEASE_USE_ITEM, BlockPos.ORIGIN, EnumFacing.DOWN));

                    if (lastSlot != reSlot)
                        PacketLib.INSTANCE.sendPacketNoEvent(new C09PacketHeldItemChange(lastSlot));
                    bowState = 2;
                }
                break;
            case 2:
                ...
                break;
            case 3:
                EntityMoveLib.INSTANCE.strafe(boostValue.get());
