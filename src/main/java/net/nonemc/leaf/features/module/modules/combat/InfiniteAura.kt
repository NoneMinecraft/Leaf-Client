...
class InfiniteAura : Module() {
        ...
        when (modeValue.get().lowercase()) {
            ...
            "click" -> {
                if (mc.gameSettings.keyBindAttack.isKeyDown) {
                    thread = thread(name = "InfiniteAura") {
                        // do it async because a* pathfinding need some time
                        val entity = Raycast.raycastEntity(
                            distValue.get().toDouble()
                        ) { entity -> entity != null && EntityTypeLib.isSelected(entity, true) } ?: return@thread
    ...
    private fun doTpAura() {
        val targets = mc.theWorld.loadedEntityList.filter {
            it is EntityLivingBase &&
                    EntityTypeLib.isSelected(it, true) &&
    ...
    private fun hit(entity: EntityLivingBase, force: Boolean = false): Boolean {
        val path = PathLib.findBlinkPath(
            mc.thePlayer.posX,
            mc.thePlayer.posY,
            mc.thePlayer.posZ,
            entity.posX,
    ...
    fun onPacket(event: PacketEvent) {
        ...
        val isMovePacket =
            (event.packet is C04PacketPlayerPosition || event.packet is C06PacketPlayerPosLook)
