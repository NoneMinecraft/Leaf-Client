    ...
    fun onAttack(event: AttackEvent) {
        if (event.targetEntity is EntityLivingBase) {
            if (event.targetEntity.hurtTime > hurtTimeValue.get() || !timer.hasTimePassed(delayValue.get().toLong()) ||
                (!EntityMoveLib.isMoving() && onlyMoveValue.get()) || (!mc.thePlayer.onGround && onlyGroundValue.get())
            ) {
                return
            }

            if (onlyMoveForwardValue.get() && RotationBaseLib.getRotationDifference(
                    Rotation(
        ...
        when (modeValue.get().lowercase()) {
            "wtap", "wtapstopmotion" -> {
                if (ticks == 2) {
                    ...
                    if (modeValue.equals("WtapStopMotion")) EntityMoveLib.resetMotion(false)
                    ticks = 1
                } else if (ticks == 1) {
