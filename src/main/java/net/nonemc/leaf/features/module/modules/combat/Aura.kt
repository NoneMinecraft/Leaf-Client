...
import net.nonemc.leaf.utils.entity.EntityUtils.isFriend
...
import net.nonemc.leaf.utils.rotation.fixRotation
import net.nonemc.leaf.utils.rotation.getRotationTo
import net.nonemc.leaf.utils.rotation.getRotationToVec
...
object Aura : Module() {
            ...
            if (rotateValue.get()) {
                currentServerYaw =
                   if (fixSensitivity.get()) {
                       fixRotation(currentServerYaw, calculateYaw(currentServerYaw, rotation.yaw, currentSpeed, it))
                   } else {
                       calculateYaw(currentServerYaw, rotation.yaw, currentSpeed, it)
                   }
            } else {
                currentYaw =
                    if (fixSensitivity.get()) {
                        fixRotation(currentYaw, calculateYaw(currentYaw, rotation.yaw, currentSpeed, it))
                    } else {
                        calculateYaw(currentYaw, rotation.yaw, currentSpeed, it)
                    }
            }
        }
    ...
    private val basicSimulationAdjustMinDelta = FloatValue(
        "BasicSimulation-AdjustMinDelta",
        10f,
        0f,
        180f
    ).displayable { rotationMode.get() == "BasicSimulation" }
    private val basicSimulationAdjustMaxDelta = FloatValue(
        "BasicSimulation-AdjustMaxDelta",
        20f,
        0f,
        180f
    ).displayable { rotationMode.get() == "BasicSimulation" }
    private val basicSimulationAdjustTime = IntegerValue(
        "BasicSimulation-AdjustTime",
        1000,
        1,
        5000
    ).displayable { rotationMode.get() == "BasicSimulation" }
    ...
    private fun calculateYaw(currentYaw: Float, targetYaw: Float, speed: Float, targetPlayer: EntityPlayer): Float {
        when (rotationMode.get()) {
            ...
            "AdvancedSimulation" -> {
                ...
                val tremorJitter =
                    Math.sin(advancedSimulationTremorPhase.toDouble()) * 0.3 * advancedSimulationTremorFrequency.get()
                        .toDouble()
                var newYaw =
