...
class CounterStrike : Module() {
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (packet is S02PacketChat) {
            ...
            if (isTeamCT && (isIronArmor || armorColorCT)) {
                playWinSound(musicMode.get(), musicNumber.get())
                ctw = true
            } else if (isTeamT && (isChainArmor || armorColorT)) {
                playWinSound(musicMode.get(), musicNumber.get())
                tw = true
            }
        }
    ...
    private fun renderCross(screenWidth: Int, screenHeight: Int, step: Int) {
        ...
        val movementSpeed =
            Math.sqrt((player.motionX * player.motionX + player.motionZ * player.motionZ).toDouble()).toFloat()
    ...
    private fun drawText1(text: String, x: Int, y: Int, r: Int, g: Int, b: Int) {
        drawText(Fonts.font28, text, x, y, r, g, b)
    }

    private fun drawText2(text: String, x: Int, y: Int, rv: Int, gv: Int, bv: Int) {
        drawText(Fonts.font28, text, x, y, rv, gv, bv)
    }

    private fun drawText3(text: String, x: Int, y: Int, rv: Int, gv: Int, bv: Int) {
        drawText(Fonts.SFUI35, text, x, y, rv, gv, bv)
    }
    ...
    fun renderImages() {
        val xOffset = Xoffset.get().toInt()
        val yOffset = YOffset.get().toInt()

        val hasM4 = findM4() != -1
        val hasAK = findAK() != -1
        when {
            hasM4 && !hasAK -> drawWeapon(
                m4,
                propx2,
                propy2 - 13,
                propw2.toDouble(),
        ...
        val hasP2000 = p2000() != -1
        val hasDeagle = findDesertEagle() != -1
        when {
            hasP2000 && !hasDeagle -> drawWeapon(
                p2000,
                propx4,
                propy4 - 25,
                propw4.toDouble(),
        ...
        if (findSword() != -1) {
            drawWeapon(sword, propx5, propy5, propw3.toDouble(), proph3.toDouble(), xOffset, yOffset)
        }
        1
        val props = listOfNotNull(
            if (findProp() != -1) prop1 else null,
            if (findProp2() != -1) prop2 else null,
            if (findProp3() != -1) prop6 else null
        )

        props.forEachIndexed { index, image ->
            RenderUtils.drawImage(
                image,
                propx.toInt() + xOffset - 10 * index,
                propy.toInt() + yOffset,
                propw.toInt(),
                proph.toInt()
            )
    ...
    private fun renderHotbar() {
        ...
        when (heldItem.item) {
            ...
            Items.blaze_powder -> {
                val (p1, p2, p3) = listOf(findProp(), findProp2(), findProp3())
                if (setOf(p1, p2, p3).filter { it != -1 }.size >= 1) {
                    RenderUtils.drawImage(
                        prop4,
            ...
            Items.carrot -> {
                ...
                RenderUtils.drawImage(
                    prop3,
                    propx.toInt() + offset + Xoffset.get().toInt(),
                    propy.toInt() + YOffset.get().toInt(),
                    propw.toInt(),
                    proph.toInt()
                )
    ...
    private fun playWinSound(musicMode: String, musicNumber: Int) {
        ...
        val sound =
            if (musicMode == "Random") soundList.random() else soundList.getOrNull(musicNumber - 1) ?: soundList.first()
        when (sound) {
