...
class CounterStrike : Module() {
   ...
   private fun drawText3(text: String, x: Int, y: Int, rv: Int, gv: Int, bv: Int){drawText(Fonts.SFUI35, text, x, y, rv, gv, bv)}
    ...
    fun renderImages() {
        ...
        val hasAK = findAK() != -1
        when {
            hasM4 && !hasAK -> drawWeapon(m4, propx2, propy2 - 13, propw2.toDouble(), proph2.toDouble(), xOffset, yOffset)
            hasAK && !hasM4 -> drawWeapon(ak, propx2, propy2 - 13, propw2.toDouble(), proph2.toDouble(), xOffset, yOffset)
        }

        val hasP2000 = p2000() != -1
        val hasDeagle = findDesertEagle() != -1
        when {
            hasP2000 && !hasDeagle -> drawWeapon(p2000, propx4, propy4 - 25, propw4.toDouble(), proph4.toDouble(), xOffset, yOffset)
            hasDeagle && !hasP2000 -> drawWeapon(DesertEagle, propx4, propy4 - 25, propw4.toDouble(), proph4.toDouble(), xOffset, yOffset)
        }
