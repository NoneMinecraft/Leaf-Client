...
class PredictionBox : Module() {
    ...
    private fun predictionBox(entity: Entity, color: Color, outline: Boolean, box: Boolean, outlineWidth: Float) {
        ...
        val x =
            if (mode.get() == "ClientSide") (entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * timer.renderPartialTicks + (mc.thePlayer.posX - mc.thePlayer.prevPosX) * predictSize.get()
                    - renderManager.renderPosX) else entity.serverPosX.toDouble() / 32
        val y =
            if (mode.get() == "ClientSide") (entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * timer.renderPartialTicks + (mc.thePlayer.posY - mc.thePlayer.prevPosY) * predictSize.get()
                    - renderManager.renderPosY) else entity.serverPosY.toDouble() / 32
        val z =
            if (mode.get() == "ClientSide") (entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * timer.renderPartialTicks + (mc.thePlayer.posZ - mc.thePlayer.prevPosZ) * predictSize.get()
