...
class Speed : Module() {
    private val modes = ClassReflect.resolvePackage("${this.javaClass.`package`.name}.speeds", SpeedMode::class.java)
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (EntityMoveLib.isMoving()) mc.thePlayer.isSprinting = true
        mode.onUpdate()
    }
    ...
    fun onMotion(event: MotionEvent) {
        if (debug.get()) {
            alert("Speed: " + EntityMoveLib.getSpeed().toString())
