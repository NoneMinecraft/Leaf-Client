...
class SnapTrack : Module() {
    ...
    private val positionProcessingMode = ListValue(
        "PositionProcessingMode",
        arrayOf("Pos", "ServerPos", "MiddlePos", "MixPos", "CustomPosOffset"),
        "MiddlePos"
    )
            ...
            if (target !== mc.thePlayer) {
                ...
                if (distance <= maxRange.get()) {
                    ...
                    if (player.getDistance(targetVecX, targetVecY, targetVecZ) <= attackRange.get()) {
                        if (!onlySwing.get() || mc.thePlayer.isSwingInProgress) target.setPositionAndUpdate(
                            targetVecX,
                            targetVecY,
                            targetVecZ
                        )
