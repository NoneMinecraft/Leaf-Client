...
class NoWeb : Module() {
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        when (modeValue.get().lowercase()) {
            ...
            "Intave" -> {
                if (EntityMoveLib.isMoving() && mc.thePlayer.moveStrafing == 0.0f) {
                    if (mc.thePlayer.onGround) {
                        if (mc.thePlayer.ticksExisted % 3 == 0) {
                            EntityMoveLib.strafe(0.734f)
                        } else {
                            mc.thePlayer.jump()
                            EntityMoveLib.strafe(0.346f)
                        }
                    }
                }
            }
            ...
            "horizon" -> {
                if (mc.thePlayer.onGround) {
                    EntityMoveLib.strafe(horizonSpeed.get())
                }
            }

            "spartan" -> {
                EntityMoveLib.strafe(0.27F)
