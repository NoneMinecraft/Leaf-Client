...
import net.nonemc.leaf.utils.extensions.ping
...
class NameTags : Module() {
    ...
    private fun getPlayerName(entity: EntityLivingBase): String {
        ...
        var pre = ""
        val teams = Leaf.moduleManager[Teams::class.java]!!
        if (Leaf.fileManager.friendsConfig.isFriend(entity.name)) {
            pre = "$pre§b[Friend] "
        }
        ...
        if (!AntiBot.isBot(entity) && !teams.isInYourTeam(entity)) {
            pre = if (Leaf.fileManager.friendsConfig.isFriend(entity.name)) {
                "§b[Friend] §c"
            } else {
                "§c"
            }
    ...
    private fun renderNameTag(entity: EntityLivingBase, tag: String) {
        ...
        } else if (onlyTarget.get() && Leaf.combatManager.target == null) {
            targetTicks--
            if (targetTicks <= -1) {
                targetTicks = 0
                entityKeep = "zywl gg"
