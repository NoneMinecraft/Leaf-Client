...
import net.nonemc.leaf.file.dataDir
...
class RotationWriter : Module() {
    ...
    private fun onUpdate(event: UpdateEvent) {
        ...
        val pitch =
            if (wrapAngleTo180.get()) MathHelper.wrapAngleTo180_float(player.rotationPitch) else player.rotationPitch
        writer?.apply {
            write("[$yaw,$pitch],")
            flush()
            chatPrint("[$yaw,$pitch]")
