    ...
    override fun execute(args: Array<String>) {
        if (args.size > 1) {
            when (args[1].lowercase()) {
                "create" -> {
                    if (args.size > 2) {
                        val file = File(Leaf.fileManager.configsDir, "${args[2]}.json")
                        if (!file.exists()) {
                ...
                "list" -> {
                    val list = (Leaf.fileManager.configsDir.listFiles() ?: return)
                        .filter { it.isFile }
                        .map {
                            val name = it.name
                ...
                "rename" -> {
                    if (args.size > 3) {
                        val file = File(Leaf.fileManager.configsDir, "${args[2]}.json")
                        val newFile = File(Leaf.fileManager.configsDir, "${args[3]}.json")
        ...
        return when (args.size) {
            ...
            2 -> when (args[0].lowercase()) {
                "delete", "load", "forceload", "rename", "copy" -> {
                    (Leaf.fileManager.configsDir.listFiles() ?: return emptyList())
                        .filter { it.isFile }
                        .map {
                            val name = it.name
                            if (name.endsWith(".json")) {
