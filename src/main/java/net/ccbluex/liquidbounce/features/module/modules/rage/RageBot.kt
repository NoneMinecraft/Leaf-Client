...
object RageBot : Module() {
    ...
    private val boundingBoxMode = ListValue("BoundingBoxDetectionMode", arrayOf("Full","4Points","8Points","12Points","4PointsCenters","Full"), "12Points").displayable { boundingBox.get() }
    ...
    private val boundingBoxStep = FloatValue("boundingBoxStep", 0.05F, 0F, 1F)
    ...
    private val acceleration = BoolValue("Acceleration", false)
    ...
    private var Stime = 0
    private var Dtime = 0
    private var SvelocityX = 0.0
    private var SvelocityY = 0.0
    private var SvelocityZ = 0.0
    private var EvelocityX = 0.0
    private var EvelocityY = 0.0
    private var EvelocityZ = 0.0
    private var DvelocityX = 0.0
    private var DvelocityY = 0.0
    private var DvelocityZ = 0.0
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        rageBotTargetPlayer?.let {it ->
            ...
            if (autoSneak.get() && autoSneakTriggerMode.get() == "OnlyFire" && (!autoSneakOnlyAwp.get() || hasWeapon(WeaponType.SNIPER))) if (autoSneakMode.get() == "Packet")
    ...
    private fun canSeePlayer(player: EntityPlayer, target: EntityPlayer): Pair<Boolean, Vec3> {
        ...
        val predictXValue = if (targetVisibilityPredict.get()) velocityX(target) * targetVisibilityPredictSize.get() else 0.0
        val predictZValue = if (targetVisibilityPredict.get()) velocityZ(target) * targetVisibilityPredictSize.get() else 0.0
        val minX = target.entityBoundingBox.minX + if (size > 0) size else 0.0 + predictXValue
        val minZ = target.entityBoundingBox.minZ + if (size > 0) size else 0.0 + predictZValue
        val maxX = target.entityBoundingBox.maxX - if (size > 0) size else 0.0 + predictXValue
        val maxZ = target.entityBoundingBox.maxZ - if (size > 0) size else 0.0 + predictZValue
        val maxY = target.entityBoundingBox.maxY
        val minY = target.entityBoundingBox.minY
        val x = target.posX
        val z = target.posZ
        val center = Vec3(x, (minY + maxY) / 2, z)
        var pointA: Vec3? = null
        var pointB: Vec3? = null
        for (x1 in generateSequence(minX) { it + boundingBoxStep.get() }.takeWhile { it <= maxX }) {
            for (y1 in generateSequence(minY) { it + boundingBoxStep.get() }.takeWhile { it <= maxY }) {
                for (z1 in generateSequence(minZ) { it + boundingBoxStep.get() }.takeWhile { it <= maxZ }) {
                    val corner = Vec3(x1, y1, z1)
                    ...
                    if (result == null || result.typeOfHit == MovingObjectPosition.MovingObjectType.MISS) {
                        if (pointA == null || corner.squareDistanceTo(center) < pointA.squareDistanceTo(center)) pointA = corner
                        if ((pointB == null || corner.squareDistanceTo(center) > pointB.squareDistanceTo(center))) pointB = corner
                    }
                }
            }
        }
        if (pointA != null && pointB != null) {
            val middlePoint = Vec3((pointA.xCoord + pointB.xCoord) / 2, (pointA.yCoord + pointB.yCoord) / 2, (pointA.zCoord + pointB.zCoord) / 2)
            return Pair(true, middlePoint)
        }
